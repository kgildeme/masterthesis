...
def main(hp_config, ds_config, experiment_config):
    ...
    try:
        model = load_model(model_path, md_train_config, device)
    ...
    logger.info("Calculate Test Scores")    
    test_scores, test_labels = get_scores(model, test_data_config, device)
    logger.info("Calculate train Scores")    
    train_scores, train_labels = get_scores(model, train_data_config, device)
    # Calculate scores for the evaluation set.
    logger.info("Calculate eval Scores")    
    eval_scores, eval_labels = get_scores(model, eval_data_config, device)
