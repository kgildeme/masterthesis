...
def main(hp_config, ds_config, experiment_config, type="train"):
    if experiment_config["loss_type"] == "baseline":
        ckpt_path =   os.path.join(misc.root(), f"models/03_semisupervision/unsupervised/{ds_config["ds_name"]}/{experiment_config["model_type"]}/{experiment_config["id"]}/{experiment_config["trial"]}/ckpt.pt")
    else:
    ...
    ds_config = copy.deepcopy(ds_config["supervised"][type])
    ...
    train_scores, train_labels = get_scores(model, data_config, device, isdeepsad="deepsad" in model_path)
    # Save full scores and labels.
    np.save(os.path.join(result_dir, f"{type}_scores.npy"), train_scores)
    np.save(os.path.join(result_dir, f"{type}_labels.npy"), train_labels)
    
    # Also save separated benign and malicious scores.
    save_scores(result_dir, type, train_scores, train_labels)
...
if __name__ == "__main__":
    ...
    assert config["experiment"]["loss_type"] in ["inverse", "reciprocal", "deepsad", "baseline"], "Loss type must be either inverse or reciprocal or deepsad"

    if config["experiment"]["loss_type"] != "baseline":
