...
def test(hp_config: dict, ds_config: dict, experiment_config: dict, binary=False):
    ...
    # create dataloader
    logger.info(f"Training with binary: {binary}")
    logger.info("Load data fro training")
    if binary:
        logger.info("Loading binary dataset")
        train_ds = get_SCVIC_DatasetBinary(train=True, validation=True, test=False, random_state=ds_config["random_state"], network_data=ds_config["network_data"],
                            host_data=ds_config["host_data"], host_embeddings=ds_config["host_embeddings"])["train"]
    else:
        train_ds = get_SCVIC_Dataset(train=True, validation=True, test=False, random_state=ds_config["random_state"], network_data=ds_config["network_data"],
                                host_data=ds_config["host_data"], host_embeddings=ds_config["host_embeddings"])["train"]
    train_ds = convert_ds(train_ds)

    logger.info("Fitting Model")
    logger.info(f"Train highest class: {np.max(train_ds[1])}")
