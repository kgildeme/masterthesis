...
from cids.eval import eval_autoregressive_encoder, eval_next_window_encoder
from cids.model import AutoregressiveTransformerEncoder
from cids.anomaly_scores import NextNAnomaly, NextWindowAnomaly
...
if __name__ == "__main__":
    ...
    logger = util.setup_logger(os.path.join(misc.root(), f"logs/debugging/eval_autoregressive_encoder_host.log"))
    device = "cuda:2"
    predict_window = False

    window = 30
    model_path = os.path.join(misc.root(), "models/debugging/host/20250112-184353/ckpt.pt")

    logger.info(f"Loading model from {model_path}")
    model_config = {
        "n_layer": 6,
        "head":nn.Linear(128, 102),
        "embedding":nn.Linear(102, 128),
        "d_model":128,
        "n_head":8,
        "dim_feedforward":256,
        "dropout":0.1,
    ...
    predict_n = 5
    tau = 1
    if not predict_window:
        anomaly = NextNAnomaly(predict_n, model, device=device)
    else:
        anomaly = NextWindowAnomaly(tau, model, device=device)
    ...
    exp_dict = "results/debugging/host/20250113-090700"
    batch_size = 128
    logger.info("Start evaluation")
    if not predict_window:
        results = eval_autoregressive_encoder(
            anomaly=anomaly,
            pred_n=predict_n,
        ...
        results = eval_next_window_encoder(
            anomaly=anomaly,
            tau=1,
            score_file_dict=os.path.join(misc.root(), exp_dict, "scores_eval"),
            data_config_train=data_config_train,
